...
export type PublicLand=Pick<Land,'id'|'title'|'location'|'region'|'governorate'|'district'|'area'|'price'|'status'|'path'> & {adminUid?:string};
...
export function publicLand(land:Land,adminUid=''):PublicLand{return {id:land.id,title:land.title,location:land.location,region:land.region||'',governorate:land.governorate||'',district:land.district||'',area:land.area,price:land.price,status:land.status,path:land.path,adminUid};}
